...
const HomePage = () => {
  ...
  // const ws = new WebSocket("ws://192.168.55.103:9001/mqtt/attendance");
  useEffect(() => {
    onClickHome();
  }, []);

  let mqtt = require("mqtt");
  let client = mqtt.connect("ws://192.168.55.106:9001/mqtt");
  let topic = "attendance/pub";

  client.on("message", (topic, message) => {
    var stringResponse = message.toString();
    var messageResponse = JSON.parse(stringResponse);
    if (!isSetting.current) {
      setCardId(messageResponse.toString());
      setTriggerNewCard((prev) => !prev);
    } else {
      setCardIdAdd(messageResponse.toString());
  ...
  return (
    <div className={styles.myContainer}>
      <div className={styles.header}>
        <span className={styles.headerTitle}>HỆ THỐNG ĐIỂM DANH SINH VIÊN</span>
      </div>
      <div className={`d-flex align-items-start`}>
        <div
          className={`nav flex-column nav-pills me-3 ${styles.tabMenu}`}
          id="v-pills-tab"
          role="tablist"
